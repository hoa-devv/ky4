 $(document).ready(function () {
	...
	const populateUI = () => {
		...
		if(idSuat1!=idSuat){
			...
			const gheChons = JSON.parse(localStorage.getItem('gheNgoi'));
			  ...
			  gia.innerText = gia1.toLocaleString('en-US') ;
	...
	$(document).on("click","#btnThanhToan",function(e){
	 	const gia = $('#gia').text().trim()?.toLocaleString('en-US');
		if( gia==0){
    ...
    const update = () => {
	 ...
	 gia.innerText = (soGheDaChon*giave1suat)?.toLocaleString('en-US');
	 const gia1 = $('#gia').text()?.toLocaleString('en-US');
